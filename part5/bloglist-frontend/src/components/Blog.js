  ...
  if(showDetails){
    return (
      <div className="transition-all relative border mb-2 p-2">
        ...
        <LikeIcon tabIndex="0" alt="like" className="inline-block mt-2 cursor-pointer w-12 h-12 bg-blue-900 p-3 rounded-2xl hover:bg-blue-500" onClick={() => addLike(blog)} />
        {blog.user.username===user[0]&&<DeleteIcon tabIndex="0" alt="delete" className="inline-block mt-2 cursor-pointer w-12 h-12 bg-red-900 p-3 rounded-2xl hover:bg-red-500 ml-2" onClick={() => deleteBlog(blog)} />}
      </div>
    )
  }
  return (
    <div onClick={toggleVisibility} className="transition-all relative border mb-2 p-2 hover:after:content-['Details'] cursor-pointer hover:after:absolute hover:after:text-white hover:after:text-center hover:after:top-0 hover:after:left-0 hover:after:p-2 hover:after:block hover:after:w-full hover:after:h-full hover:after:bg-blue-900/40">
